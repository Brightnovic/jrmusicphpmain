...
<body class="">
            ...
            <div class="container">
                ...
                <div class="container py-2">
                    <div class="row">
                        ...
                        <div class="col img-color">
                            <a href="./music.php" class="text-light fw-bolder"> <i class="fas fa-music"></i>music</a>
                        </div>
                        ...
                        <div class="col img-color">
                            <a href="./video.php" class="text-light fw-bolder"> <i class="fas fa-video"></i>
                                </i>video</a>
                        </div>
                    </div>
                </div>
 ...
 -->
    ...
    <main class="container mediacontainer">
        ...
        <?php $to_download = get_song_by_name($_GET['song']); ?>

        <div class="info my-5">
            <h1 class="h1 text-primary capitar-text"><?= $to_download['artist']; ?></h1>
            <h1 class=" h3 capitar-text"><?= $to_download['song_name']; ?></h1>
        </div>

        <!--     img  div-->
        <div class="imgMusic py-3">
            <?php if ($to_download['song_photo']) : ?>
                <img src="../photos/<?= json_decode($to_download['song_photo'], true)['name']; ?>" alt="music background">
            <?php else : ?>
                <video controls src="../videos/<?= json_decode($to_download['song'], true)['name']; ?>" alt="unable to load music image" srcset=""></video>
            <?php endif; ?>
        </div>
        <!-- infoMedia -->

        <div class="media py-3 center">
            <?php if ($to_download['song_photo'] == null) : ?>
                <p></p>
            <?php else : ?>
                <audio src="../uploads/<?= json_decode($to_download['song'], true)['name']; ?>" controls></audio>
            <?php endif; ?>
        </div>
        <div>
        <?php if ($to_download['song_photo'] == null) : ?>
            <form class="py-3" action="./download.php?video=<?= json_decode($to_download['song'], true)['name'] ?>" method="post">
                <button class="btn-primary fw-bold py-2 rounded" name="download" value="<?php $to_download['song_id'] ?>"> Download </button>
            </form>
            <?php else : ?>
                <form class="py-3" action="./download.php?song=<?= json_decode($to_download['song'], true)['name'] ?>" method="post">
                <button class="btn-primary fw-bold py-2 rounded" name="download" value="<?php $to_download['song_id'] ?>"> Download </button>
            </form>
            <?php endif; ?>
        </div>
