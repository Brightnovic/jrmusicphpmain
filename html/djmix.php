...
<body class="">
    ...
    <div class="container bg-dark my-3 bg-show py-4">
        <div class="col text-capitalize text-lg-center text-light text-sm-start my-2 bg fw-bolder display-5">Dj Mix</div>
    </div>
    ...
    <div class="container  text-white my-3  ">
        ...
        <div class="col py-3  row mt-3 d-flex my-3 ">
            <?php $DJ = get_songs_by_genre("dj_mix", $start, $song_per_page); ?>

            <?php if ($DJ) : ?>
                <?php foreach ($DJ as $dj_mix) : ?>
                    <div class="newdiv border border-primary d-block d-flex flex my-3 ">
                        <div id="img" class="circlerounded col-2">
                            <img src="../photos/<?= json_decode($dj_mix['song_photo'], true)['name']; ?>" alt="unable to load music image" srcset="" class="imgme img img-fluid font-italic text-danger">
                        </div>

                        <div class="content py-3 d-flex my-3">
                            <div id="artise" class=" text-primary fw-bold my-3">
                                <!-- song title and artist -->
                                <p><?= $dj_mix['artist']; ?> - <?= $dj_mix['song_name']; ?></p>

                                <!-- download link -->
                                <form method="post" action="./uploadedcontent.php?song=<?= $dj_mix['song_name']; ?>">
                                    <button name="download" value="<?= json_decode($dj_mix['song'], true)['name']; ?>" class=" link link-offset-3">download MP3 </button>
                                </form>
                            </div>
                        </div>
                    </div>
