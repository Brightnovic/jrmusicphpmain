...
<head>
    ...
    <title>Comedy Videos Page</title>
</head>
...
<body class="">
    ...
    <div class="container bg-dark my-3 bg-show py-4">
        <div class="col text-capitalize text-lg-center text-light text-sm-start my-2 bg fw-bolder display-5">Comedy videos</div>
    </div>
    ...
    <div class="container  text-white my-3  ">
        ...
        <div class="col py-3  row mt-3 d-flex my-3 ">
            <?php $MUSIC_VIDEO = get_songs_by_genre("comedy", $start, $song_per_page); ?>

            <?php if ($MUSIC_VIDEO) : ?>
                <?php foreach ($MUSIC_VIDEO as $music_video) : ?>
                    <div class="newdiv border border-primary d-block d-flex flex my-3 ">
                        <div id="img" class="circlerounded col-2">
                            <video src="../videos/<?= json_decode($music_video['song'], true)['name']; ?>" alt="unable to load music image" srcset="" class="imgme img img-fluid font-italic text-danger"></video>
                        </div>

                        <div class="content py-3 d-flex my-3">
                            <div id="artise" class=" text-primary fw-bold my-3">
                                <!-- song title and artist -->
                                <p><?= $music_video['artist']; ?> - <?= $music_video['song_name']; ?></p>

                                <!-- download link -->
                                <form method="post" action="./uploadedcontent.php?song=<?= $music_video['song_name']; ?>">
                                    <button name="download" value="<?= json_decode($music_video['song'], true)['name']; ?>" class=" link link-offset-3">download video </button>
                                </form>
                            </div>
                        </div>
                    </div>
