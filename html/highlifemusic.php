...
<body class="">
    ...
    <div class="container text-white">
        ...
        <div class="col py-3  row mt-3 d-flex my-3 ">
            <?php $HIGHLIFE = get_songs_by_genre("highlife", $start, $song_per_page); ?>

            <?php if ($HIGHLIFE) : ?>
                <?php foreach ($HIGHLIFE as $high) : ?>
                    <div class="newdiv border border-primary d-block d-flex flex my-3 ">
                        <div id="img" class="circlerounded col-2">
                            <img src="../photos/<?= json_decode($high['song_photo'], true)['name']; ?>" alt="unable to load music image" srcset="" class="imgme img img-fluid font-italic text-danger">
                        </div>

                        <div class="content py-3 d-flex my-3">
                            <div id="artise" class=" text-primary fw-bold my-3">
                                <!-- song title and artist -->
                                <p><?= $high['artist']; ?> - <?= $high['song_name']; ?></p>

                                <!-- download link -->
                                <form method="post" action="./uploadedcontent.php?song=<?= $high['song_name']; ?>">
                                    <button name="download" value="<?= json_decode($high['song'], true)['name']; ?>" class=" link link-offset-3">download MP3 </button>
                                </form>
                            </div>
                        </div>
                    </div>
